  ...
  return (
    ...
    >
      <div className="relative mix-blend-overlay">
        ...
        <div className="mx-auto lg:flex md:flex max-w-7xl items-center justify-between px-6 lg:px-8">
          <div className="md:py-4 sm:p-8 lg:grid lg:grid-cols-2 lg:static lg:max-h-full bg-white lg:overflow-visible rounded-lg shadow-lg dark:bg-gray-700 bg-opacity-80 dark:bg-opacity-70">
            ...
            <div className="p-6 flex flex-col z-50 text-amber-950 dark:text-amber-200 text-xl lg:text-2xl md:text-2xl justify-evenly">
                ...
                <h3><span className='underline font-bold'>Round {circuitList.length > 0 && circuitList[selectedCircuitIndex].id}:</span> <span className="text-sm lg:text-xl md:text-xl">{circuitList.length > 0 && circuitList[selectedCircuitIndex].name}</span></h3>
                <h3><span className='underline font-bold'>Location:</span> <span className="text-sm lg:text-xl md:text-xl">{circuitList.length > 0 &&  circuitList[selectedCircuitIndex].location}, {circuitList.length > 0 && circuitList[selectedCircuitIndex].country}</span></h3>
                <h3><span className='underline font-bold'>Date</span>: <span className="text-sm lg:text-xl md:text-xl">{circuitList.length > 0 &&circuitList[selectedCircuitIndex].date}</span></h3>
            </div>
        ...
        <ItemSelectorHScroll ref={containerRef} dataList={circuitList} idx={selectedCircuitIndex} handleImageHover={handleImageHover} handleImageClick={handleImageClick} /> 
        </div>
      </div>
    </motion.div>
  )
